        ...
        if not reports_df.empty:
            merged_df = merged_df.merge(
                reports_df[['diagnostic_report_id', 'report_status', 'report_conclusion', 
                           'report_issued', 'report_effective_period_start', 'report_effective_period_stop', 'category_text']],
                left_on='result_diagnostic_report_id',
                right_on='diagnostic_report_id',
                how='left'
            )
            logger.info(f"  ✅ Merged diagnostic reports ({len(reports_df)} reports with date fields + category_text)")
