...
def consolidate_matches(all_matches, medications_df):
    ...
    chemo_df = chemo_df.sort_values(
        by=['strategy_count', 'medication_start_date'],
...
def generate_summary(chemo_df, matched_indices, medications_df, output_file):
    ...
    if not chemo_df.empty:
        logger.info(f"\nTemporal Coverage:")
        logger.info(f"  First chemotherapy order: {chemo_df['medication_start_date'].min()}")
        logger.info(f"  Last chemotherapy order: {chemo_df['medication_start_date'].max()}")
