#!/usr/bin/env python3
"""
Extract Radiation Oncology Data from Athena FHIR Database (Config-Driven)
...
- Radiation oncology documents (PDFs, images, reports)
- Treatment timeline reconstruction
- Re-irradiation identification

**Config-Driven Workflow**: 
- Loads patient details from patient_config.json
- No command-line arguments needed
- Outputs to staging_files/patient_{shortid}/
...
- document_reference (Rad Onc documents with retrieval flags)

Column Naming Convention:
- Resource prefixes: cp_, cpn_, cppo_, sr_, srn_, srrc_, proc_, pcc_, pn_, doc_
...
Date: 2025-10-12
Updated: 2025-10-12 - Refactored to config-driven pattern
"""
...
# NOTE: Configuration loaded from patient_config.json (no hardcoded values)
...
def main():
    """Main extraction workflow using patient_config.json."""
    ...
    if not config_file.exists():
        ...
        print("\nPlease create patient_config.json with:")
        print("""{
  "fhir_id": "PATIENT_FHIR_ID",
  "birth_date": "YYYY-MM-DD",
  "output_dir": "staging_files/patient_SHORTID",
  ...
  "s3_output": "s3://aws-athena-query-results-343218191717-us-east-1/"
    ...
    patient_id = patient_fhir_id  # Store original for bare ID usage
    database = config.get('database', 'fhir_prd_db')
    ...
    if not patient_fhir_id.startswith('Patient/'):
        patient_fhir_ref = f'Patient/{patient_fhir_id}'
    else:
        patient_fhir_ref = patient_fhir_id
        patient_id = patient_fhir_id.replace('Patient/', '')
    
    print("="*80)
    print("RADIATION THERAPY DATA EXTRACTION (Config-Driven)")
    print("="*80)
    print(f"\nPatient FHIR ID: {patient_id}")
    print(f"FHIR Reference: {patient_fhir_ref}")
    ...
    # Store config values in global scope for use by helper functions
    global DATABASE, S3_OUTPUT, OUTPUT_DIR
    DATABASE = database
    S3_OUTPUT = s3_output
    OUTPUT_DIR = output_dir
    
    # Extract data
    consults_df = extract_radiation_oncology_consults(athena, patient_fhir_ref)
    treatments_df = extract_radiation_treatment_appointments(athena, patient_fhir_ref)
    ...
    # Save outputs (use output_dir from config)
    ...
    if len(consults_df) > 0:
        consults_file = output_dir / 'radiation_oncology_consults.csv'
        consults_df.to_csv(consults_file, index=False)
        print(f"  ✅ Saved: radiation_oncology_consults.csv ({len(consults_df)} rows)")
    
    # Save treatments
    if len(treatments_df) > 0:
        treatments_file = output_dir / 'radiation_treatment_appointments.csv'
    ...
    if len(courses) > 0:
        ...
        courses_file = output_dir / 'radiation_treatment_courses.csv'
    ...
    if len(care_plan_notes_df) > 0:
        notes_file = output_dir / 'radiation_care_plan_notes.csv'
    ...
    if len(care_plan_hierarchy_df) > 0:
        hierarchy_file = output_dir / 'radiation_care_plan_hierarchy.csv'
