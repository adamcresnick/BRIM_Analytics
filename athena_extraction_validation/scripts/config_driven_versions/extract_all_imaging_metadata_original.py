#!/usr/bin/env python3
"""
Extract ALL imaging studies with comprehensive metadata for patient C1277724
...
# Output file
OUTPUT_FILE = STAGING_DIR / f'ALL_IMAGING_METADATA_{PATIENT_MRN}.csv'
...
class ImagingExtractor:
    ...
    def extract_mri_results(self):
        ...
        WHERE EXISTS (
            ...
            AND mri.patient_id = '{PATIENT_FHIR_ID}'
        )
        """
    ...
    def merge_all_data(self, mri_df, mri_results_df, other_imaging_df, reports_df):
        ...
        try:
            birth_dt = pd.to_datetime(BIRTH_DATE).tz_localize(None)  # Remove timezone
...
def main():
    ...
    logger.info(f"\n✅ Saved {len(final_df)} imaging studies to {OUTPUT_FILE}")
