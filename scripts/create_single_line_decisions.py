#!/usr/bin/env python3
"""
Create decisions.csv in BRIM format with SINGLE-LINE entries (no embedded newlines)

CRITICAL: BRIM's CSV parser cannot handle multi-line fields even if properly quoted.
All prompt_template content must be on a single line with spaces instead of newlines.
"""

import csv

def create_single_line_decisions(output_file):
    """Create decisions CSV with single-line entries for BRIM compatibility"""
    ...
    decisions = [
        ...
        },
        {
            ...
            'decision_type': 'text',
            'prompt_template': 'Return the extent of resection for the FIRST surgery. FILTERING LOGIC: 1. Identify the EARLIEST value in surgery_date (first surgery date) 2. Find surgery_extent values associated with that date 3. Prioritize document_type=OPERATIVE (surgeons assessment is most authoritative). VALID RETURN VALUES: Must be one of: Gross Total Resection, Near Total Resection, Subtotal Resection, Partial Resection, Biopsy Only, Unknown. Gold Standard for C1277724 Surgery 1 (2018-05-28): Partial Resection. Return ONLY the extent classification.',
        ...
        },
        {
            ...
            'decision_type': 'text',
            'prompt_template': 'Return the anatomical location for the FIRST surgery. FILTERING LOGIC: 1. Identify the EARLIEST value in surgery_date (first surgery date) 2. Find surgery_location values associated with that date 3. Use most specific location mentioned. Gold Standard for C1277724 Surgery 1 (2018-05-28): Cerebellum/Posterior Fossa. Return ONLY the location name.',
        ...
        },
        {
            ...
            'decision_type': 'text',
            'prompt_template': 'Return the anatomical location for the SECOND surgery (if it exists). FILTERING LOGIC: 1. Check if surgery_number >= 2 2. If yes: Identify SECOND EARLIEST surgery_date 3. Find surgery_location associated with that date 4. If surgery_number < 2: Return "Unknown". Gold Standard for C1277724 Surgery 2 (2021-03-10): Cerebellum/Posterior Fossa. Return ONLY the location name.',
        ...
        # Aggregation decisions
        {
            ...
            'prompt_template': 'Count the total number of distinct surgeries for this patient. AGGREGATION LOGIC: Count the number of unique values in surgery_date variable. This should match the surgery_number variable. Gold Standard for C1277724: 2. Return ONLY the integer count.',
        ...
        },
        {
            ...
            'decision_type': 'text',
            'prompt_template': 'Aggregate all chemotherapy agents into a semicolon-separated list. AGGREGATION LOGIC: 1. Collect ALL chemotherapy_agents values extracted across all documents 2. Remove duplicate agent names 3. Return as semicolon-separated list. Gold Standard for C1277724: vinblastine;bevacizumab;selumetinib. Return in format: "agent1;agent2;agent3"',
        ...
        },
        {
            ...
            'decision_type': 'text',
            'prompt_template': 'Aggregate all documented symptoms into a semicolon-separated list. AGGREGATION LOGIC: 1. Collect ALL symptoms_present values across all documents 2. Remove duplicates 3. Return as semicolon-separated list. Gold Standard for C1277724: Emesis;Headaches;Hydrocephalus;Visual deficit;Posterior fossa syndrome. Return in format: "symptom1;symptom2;symptom3"',
        ...
        },
        {
            ...
            'decision_type': 'text',
            'prompt_template': 'Create comprehensive molecular profile summary. AGGREGATION LOGIC: Combine all molecular test results into single text summary. Format: "BRAF: [status]; IDH: [status]; MGMT: [status]; Tests: [list]". Gold Standard for C1277724: BRAF fusion present (KIAA1549-BRAF). Return structured summary.',
        ...
        },
        {
            ...
            'decision_type': 'text',
            'prompt_template': 'Create timeline of imaging findings showing disease course. AGGREGATION LOGIC: 1. Extract ALL imaging_findings with their document dates 2. Prioritize document_type=IMAGING 3. Sort chronologically 4. Return as semicolon-separated list with dates. Gold Standard for C1277724: Multiple imaging studies documenting progression. Return in format: "YYYY-MM-DD: finding;YYYY-MM-DD: finding"',
    ...
    with open(output_file, 'r') as f:
        csv_rows = len(list(csv.reader(f))) - 1  # -1 for header
    
    print(f"✅ Created {output_file}")
    print(f"   CSV rows: {csv_rows}")
    print(f"   File lines: {file_lines}")
    print(f"   Format: {'✅ SINGLE-LINE (correct)' if file_lines == csv_rows + 1 else '❌ MULTI-LINE (wrong)'}")
    
    return decisions

if __name__ == "__main__":
    output_file = "/Users/resnick/Documents/GitHub/RADIANT_PCA/BRIM_Analytics/pilot_output/brim_csvs_iteration_3c_phase3a_v2/decisions.csv"
    
    print("=" * 80)
    print("CREATING SINGLE-LINE DECISIONS.CSV FOR BRIM")
    print("=" * 80)
    print()
    print("CRITICAL FIX: Removing all embedded newlines from prompt_template field")
    print("BRIM's CSV parser requires single-line format")
    print()
    ...
    decisions = create_single_line_decisions(output_file)
    
    print()
    print("=" * 80)
    print("✅ COMPLETE - decisions.csv ready for BRIM upload")
    print("=" * 80)
