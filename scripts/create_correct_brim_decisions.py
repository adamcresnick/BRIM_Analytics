...
def create_correct_brim_decisions(output_file):
    ...
    decisions = [
        ...
        # Aggregation decisions
        {
            ...
            'instruction': 'Count the total number of distinct surgeries for this patient. AGGREGATION LOGIC: Count the number of unique values in surgery_date variable. This should match the surgery_number variable. Gold Standard for C1277724: 2. Return ONLY the integer count.',
        ...
        },
        {
            ...
            'instruction': 'Aggregate all chemotherapy agents into a semicolon-separated list. AGGREGATION LOGIC: 1. Collect ALL chemotherapy_agents values extracted across all documents 2. Remove duplicate agent names 3. Return as semicolon-separated list. Gold Standard for C1277724: vinblastine;bevacizumab;selumetinib. Return in format: "agent1;agent2;agent3"',
            'decision_type': 'text',
            'prompt_template': standard_prompt_template,
        ...
        },
        {
            ...
            'instruction': 'Create comprehensive molecular profile summary. AGGREGATION LOGIC: Combine all molecular test results into single text summary. Format: "BRAF: [status]; IDH: [status]; MGMT: [status]; Tests: [list]". Gold Standard for C1277724: BRAF fusion present (KIAA1549-BRAF). Return structured summary.',
            'decision_type': 'text',
            'prompt_template': standard_prompt_template,
    ...
    with open(output_file, 'r') as f:
        csv_rows = len(list(csv.reader(f))) - 1
    
    print(f"✅ Created {output_file}")
    print(f"   CSV rows: {csv_rows}")
    print(f"   File lines: {file_lines}")
    print(f"   Format: {'✅ SINGLE-LINE' if file_lines == csv_rows + 1 else '❌ MULTI-LINE'}")
    print(f"   Variables format: JSON array with quoted strings [\"var1\", \"var2\"]")
    print(f"   Prompt template: Uses {{instruction}}, {{name}}, {{decision_type}} placeholders")
...
if __name__ == "__main__":
    ...
    print("=" * 80)
    print("CREATING DECISIONS.CSV IN CORRECT BRIM FORMAT")
    print("=" * 80)
    print()
    print("Based on CSK example file format:")
    print("  - Variables: JSON array with quoted strings")
    print("  - Prompt template: Standard JSON output format with placeholders")
    print()
    ...
    decisions = create_correct_brim_decisions(output_file)
