...
def match_by_dob_and_name(cbtn_row, demo_df, match_stats, stat_prefix=''):
    """
...
def process_cbtn_enrollment_generic(input_csv, output_csv, db_configs):
    ...
    Args:
        db_configs: List of tuples (database_label, mrn_map, demo_df)
    """
    ...
    required_cols = ['mrn', 'dob', 'first_name', 'last_name']
    ...
    for db_label, _, _ in db_configs:
    ...
    for idx, row in df.iterrows():
        ...
        for db_label, mrn_map, demo_df in db_configs:
            if matched:
                break
            
            # Strategy 1: Try MRN exact match first
            if mrn and mrn in mrn_map:
                df.at[idx, 'FHIR_ID'] = mrn_map[mrn]
            ...
            if not demo_df.empty:
                fhir_id, match_type = match_by_dob_and_name(row, demo_df, stats, stat_prefix=f'{db_label}_')
...
def main():
    ...
    try:
        ...
        db_configs.append(('chop', chop_mrn_map, chop_demo_df))
        ...
        if not args.skip_ucsf:
            ...
            try:
                ...
                db_configs.append(('ucsf', ucsf_mrn_map, ucsf_demo_df))
            except Exception as e:
